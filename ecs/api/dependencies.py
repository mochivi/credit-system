...
from ecs.core.security import verify_access_token, verify_client_access_token
...
def get_current_principal(token: Annotated[str, Depends(oauth2_scheme)]) -> TokenData:
    ...
    bind_contextvars(principal_sub=token_data.sub, principal_type=token_data.typ)
    return token_data

def get_current_client_principal(token: Annotated[str, Depends(oauth2_scheme)]) -> TokenData:
    token_data: TokenData = verify_client_access_token(token)
    bind_contextvars(principal_sub=token_data.sub, principal_type=token_data.typ)
    return token_data

CurrentPrincipalDep: TypeAlias = Annotated[TokenData, Depends(get_current_principal)]
ClientPrincipalDep: TypeAlias = Annotated[TokenData, Depends(get_current_client_principal)]
